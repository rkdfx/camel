...
The `maxAgenticTokens` option provides an additional, cost-oriented safeguard: it caps the cumulative number of tokens
(prompt + completion, summed across every iteration) the loop may consume. It is unset by default (unlimited). When the
budget is exceeded the loop is halted before the next model call and an `IllegalStateException` is thrown, mirroring
`maxToolIterations`. The cumulative usage is also exposed via the `CamelOpenAIAgentic*Tokens` headers (see below).
...
| `CamelOpenAIAgenticPromptTokens` | Long | Cumulative prompt tokens consumed across all iterations
| `CamelOpenAIAgenticCompletionTokens` | Long | Cumulative completion tokens consumed across all iterations
| `CamelOpenAIAgenticTotalTokens` | Long | Cumulative total tokens (prompt + completion) consumed across all iterations
|===
...
| Token budget (`maxAgenticTokens`) exceeded | `IllegalStateException` is thrown with the cumulative token usage and tool call log
